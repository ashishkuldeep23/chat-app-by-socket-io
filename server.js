...
app.use((req, res) => {
  res
    .status(404)
    .send(
      ' <div> <img src="http://www.digitalmesh.com/blog/wp-content/uploads/2020/05/404-error.jpg" alt="page not found image" style="height:100vh; width:100vw;" /></div> '
    );
});
...
const OWN_SERVER_URL =
  process.env.OWN_SERVER_URL || "http://localhost:3000/alive";
...
io.on("connection", (socket) => {
  ...
  socket.on("connected-new", (userDetails) => {
    ...
    socket.broadcast.emit("oneUserPlus", {
  ...
  socket.on("typing_send", (typingObj) => {
    ...
    if (typingObj.len >= 1) {
      socket.broadcast.emit("typing_recive", {
        user: typing,
        len: typingObj.len,
      });
    }
